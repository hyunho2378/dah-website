...
## 44_EXHIBITION_DATE_FIX — 전시회 폼 날짜 필드 재배치 + 종료일 동기화
- [x] **필드 순서**: `PostForm.jsx` 전시회 템플릿에서 전시 사이트 URL(좌)–시작일(우)/종료일(좌) 배치를 시작일(좌)–종료일(우)로 바꾸고, 전시 사이트 URL을 그 아래 전체 폭(`md:col-span-2`) 한 줄로 내렸다. 다른 필드는 순서·마크업 무변경
- [x] **종료일 자동 채움**: 시작일 전용 핸들러 `setStartDate` 추가 — 종료일이 비었거나 시작일보다 이르면 시작일로 맞춘다. 값이 `YYYY-MM-DD` 고정 폭이라 문자열 비교가 곧 날짜 비교다. 종료일이 시작일보다 늦으면 손대지 않는다
...
## 47_CONTEST_EDITIONS_ADMIN — 공모전 회차 개별 편집
- [!] **진단 정정**: 회차 리피터(`EditionsField`, M1-3)는 이미 있었다. 공모전 수정 화면은 상단에 제목·영문명(공용 블록)·주최, 그 아래 회차 카드 목록 구조였고 학기 라벨·포스터(ImageUpload 미리보기 포함)·링크 편집과 회차 추가·삭제도 동작했다. 실제 미비는 두 가지였다 — 기간이 DatePicker가 아닌 자유 입력, 대시보드 목록에 회차 수 미표시
- [x] **기간 DatePicker화**: 회차 카드의 기간을 자유 입력 1칸에서 시작일·종료일 `DateInput` 2칸으로 바꿨다. 종료일에는 `viewDate={row.start_date}`를 물려 시작일 달에서 캘린더가 열린다(44에서 추가한 prop 재사용)
- [x] **공개 렌더 계약 유지**: 공개 페이지(`Contests.jsx`·`ContestDetail.jsx`)는 예전처럼 문자열 `period`만 읽는다. 날짜를 편집할 때 `period`를 그 자리에서 `시작 ~ 종료`로 파생해 저장하므로 공개 페이지 코드는 무변경이다. 한쪽만 고르면 그 날짜 하나, 둘 다 비우면 빈 문자열
- [x] **레거시 기간 보존**: 날짜가 없고 문구만 있는 기존 회차는 `period` 원문을 건드리지 않고, 카드에 "기존 기간 표기: … (시작일·종료일을 고르면 대체됩니다)"로 노출해 값이 조용히 사라지지 않게 했다
- [x] **카드 식별성**: 카드 헤더를 "회차 N"에서 "회차 N · 학기 라벨"로 바꿔 접힌 목록에서도 어느 학기인지 보이게 했다. 학기 라벨은 전체 폭으로 올렸다
- [x] **대시보드 회차 수**: `postTypes.js`의 `metaOf`가 `type === 'contest'`이면 날짜 대신 `N회차`를 반환한다. 어드민 목록 API가 `body`를 내려주는 것을 확인하고 붙였다
- [x] **검증**: 클라 빌드 통과. `period` 파생 규칙 6케이스 자체 점검 통과(시작만·시작+종료·시작 삭제·종료 삭제·날짜 미편집 시 레거시 보존·전부 삭제). 어드민 API 실측 — `GET /admin/content/contest/175`가 회차 4개를 그대로 내려주고(카드 4장 프리필 소스), 목록은 `4회차 / 4회차 / (회차 없음)`으로 표시됨
- [!] **미검증(브라우저 도구 부재)**: 어드민 편집 화면에서 회차 카드 4장이 실제로 그려지고 저장까지 왕복하는지 육안 확인은 못 했다. API 응답·빌드·파생 로직까지만 대조했다

## 48_CONTEST_EDITION_TITLE — 회차 제목 필드 + 회차별 카드 렌더
- [x] **T1 editions.title**: `body.editions[]`에 `title` 문자열 추가. 어드민 회차 카드(`EditionsField`) 최상단에 "회차 제목" 입력 칸 신설(힌트: 비우면 공모전 제목과 학기로 대체)
- [x] **T2 DB 갱신**: id 175·176의 editions 8개를 지정 원문 그대로 UPDATE. `poster_url`은 기존 값을 학기 접두사로 매칭해 승계했고, `link`·날짜 등 기존 필드도 전개 보존. 학기 라벨은 지시대로 `2024-2` 형태(기존 `2024-2학기`에서 변경). 트랜잭션 + 회차 4개 검증 후 커밋, 불일치 시 롤백. `seed-contests.mjs`도 동일 내용으로 갱신(장서표는 학기마다 주관 도서관이 달라 기계 파생이 불가능해 원문 배열로 나열)
- [x] **T3 회차별 카드**: 카드에 `edition.title` 제목 표시(없으면 `학기 + 공모전 제목` 폴백). 정렬은 학기 라벨 문자열 내림차순으로 최신(2026-1)이 맨 앞. 라벨이 `YYYY-N` 고정 폭이라 문자열 비교가 곧 학기 비교다
- [x] **T4 단발성 공모전**: editions가 없으면 기존 폴백(공모전 자신을 1회차로) 그대로. 캐릭터 공모전(id 23)은 카드 1장 유지 확인
- [x] **T5 상세 페이지**: `ContestDetail.jsx`는 editions를 포스터 폴백에만 쓰고 목록을 렌더하지 않았다. 갤러리 아래에 회차 섹션을 추가해 회차별 포스터·학기·제목·기간·링크를 카드로 나열. i18n 키 `sections.editions`(회차 / Editions) 신설
- [x] **검증**: 클라 빌드 통과. 공개 API 실측으로 회차 8건의 학기·제목을 지정 원문과 1:1 대조 — **불일치 0건**, 포스터 파일 8건 실재, 화면 정렬 적용 시 2026-1 → 2025-2 → 2025-1 → 2024-2 순, 총 카드 9장(4+4+1)
- [!] **진단 정정**: "현재는 공모전 1건 = 카드 1장"은 실제와 달랐다. `Contests.jsx`는 이전부터 editions를 풀어 회차마다 `EditionCard`를 그렸다(공모전 제목·주최 블록 아래 그리드). 이번에 바뀐 것은 카드에 회차 제목이 붙은 것과 정렬이 최신 우선으로 뒤집힌 것이다. 공모전 블록의 제목·주최 헤더는 그대로 두었다 — 제거 지시가 없었고 캐릭터 공모전 폴백도 이 구조에 기댄다
- [!] **미검증(브라우저 도구 부재)**: 실제 렌더와 카드 레이아웃 육안 확인은 못 했다. API 응답·빌드까지만 대조했다

## 49_CONTEST_EDITION_DETAIL — 공모전 회차별 상세 페이지
- [x] **라우트 신설**: `/programs/contests/:id/:edition` 추가. 기존 `/programs/contests/:id`(공모전 상세)와 세그먼트 수가 달라 그대로 공존한다
- [x] **회차 키는 학기 라벨**: `:edition`은 `semester_label`을 우선 매칭하고 없거나 못 찾을 때만 배열 인덱스로 떨어진다. 목록 카드는 최신 학기순으로 정렬해 보여주므로 정렬 후 순번으로 링크를 걸면 원본 배열과 어긋난다 — `Contests.jsx`도 정렬 전에 원본 인덱스를 함께 들고 가도록 `{ed, index}` 형태로 바꿔 폴백 경로까지 어긋나지 않게 했다
- [x] **`ContestEditionDetail.jsx` 신설**: ContestDetail과 같은 패턴(좌 포스터 2:3 / 우 정보). 회차 제목 h1, 포스터, 주최(상위 공모전 host), 학기·기간 메타 표, 외부 링크 버튼(`edition.link`, 새 탭), 공유 버튼, 하단 "목록으로" 링크
- [x] **영문 지원**: `edition.title_en`이 있으면 EN 모드에서 표시하고, 없으면 국문 제목 + Korean only 뱃지. 어드민 회차 카드에 "회차 제목 (영문)" 입력 칸도 함께 추가했다(T1에서 국문 title만 넣었던 것 보완)
- [x] **단발성 공모전 유지**: editions가 없으면 카드 링크가 기존 `/programs/contests/:id`로 가고 상세도 기존 `ContestDetail` 그대로. 캐릭터 공모전(id 23) 확인
- [x] **검증**: 클라 빌드 통과. 회차 매칭·정렬 7케이스 자체 점검 통과(최신·최구 라벨, 인덱스 폴백, 범위 밖, 미존재 라벨, 정렬 순서, 정렬 후 원본 인덱스 보존). 실제 API로 회차 8개 URL을 모두 조회해 **URL → 회차 제목·학기·포스터가 1:1로 정확히 매칭**됨을 확인
- [!] **미검증(브라우저 도구 부재)**: 카드 클릭 → 상세 이동과 실제 렌더 육안 확인은 못 했다. 라우트 등록·매칭 로직·API 응답·빌드까지만 대조했다

## 50_EDITIONS_ACCORDION — 공모전 회차 카드 접이식 압축 (UI 전용)
- [x] **배경**: 회차 4개는 이전부터 정상 렌더되고 있었다(49 세션에서 DB·운영 API·어드민 API·배포 번들·저장 왕복 전 층 실측 완료). 다만 47~49에서 회차 제목·영문 제목·학기를 각각 전체 폭 행으로 올리고 기간을 2칸으로 쪼개면서 카드 1장이 두 배 넘게 길어져, 4장이면 세로 2500px를 넘어 첫 카드만 보이고 나머지가 묻혔다. 이번 작업은 **UI 압축만** 하고 데이터 경로는 건드리지 않았다
- [x] **접이식 헤더**: 기본 접힘. 헤더 한 줄에 순번·학기 라벨·회차 제목(제목 없으면 "제목 없음")과 ChevronDown(펼침 시 rotate-180). `py-12` + `text-body-m`이라 행 높이가 44px대라 4회차가 한 화면에 들어온다. 삭제 버튼은 헤더 우측에 유지
- [x] **2열 그리드**: 펼친 패널은 `md:grid-cols-2`로 국문 제목·영문 제목 / 학기 라벨·링크 / 시작일·종료일 3줄 + 포스터 전체 폭. 기존 7줄 전체 폭에서 4줄로 줄었다. 모바일은 1열
- [x] **회차 추가**: 목록 맨 아래 유지. 추가 시 `setOpenIndex(rows.length)`로 새 회차가 펼친 상태로 시작한다. 삭제 시에는 순번이 밀리므로 그냥 닫는다(단일 열림 아코디언)
- [x] **정상 로직 무변경**: `rows.map(...)` 전체 순회, `Array.isArray(value)` 가드, `onChange([...rows, {}])` 추가, `onChange(rows.filter(...))` 삭제, `setRow`의 period 파생까지 그대로다. diff 확인 결과 바뀐 것은 콜백이 블록 본문이 된 것과 JSX 마크업뿐
...
## 52_CONTEST_CATEGORY — 공모전 종류 필드 + 종류별 섹션 + 학기 라벨 통일
- [x] **스키마**: `posts`에 `category`·`category_etc` 추가(`ADD COLUMN IF NOT EXISTS`, DROP 없음). `category`는 정해진 3종 중 하나이고 `category_etc`는 "기타"일 때의 직접 입력값이다. 섹션 묶음은 `category`로만 하므로 직접 입력값이 달라도 같은 기타 섹션에 모인다. `schema.sql` + `migrate-phase52.mjs` 양쪽 반영
- [x] **백필**: 제목 매칭으로 채웠다 — `%장서표%` 4건 → 도서관 장서표 디자인 공모전, `%포스터 공모전%` 4건 → 디지털인문예술 프로젝트 전시회 포스터 공모전, 나머지 1건(캐릭터) → 기타 + `category_etc` "신규 캐릭터 공모전". `category IS NULL`인 행만 채우므로 재실행해도 기존 값을 덮어쓰지 않는다
- [x] **종류 값 단일 출처**: 서버 `contest-data.mjs`의 `CONTEST_CATEGORY`와 클라 `data/contestCategory.js`가 같은 문자열을 쓴다. DB에 문자열이 그대로 들어가므로 한쪽만 바꾸면 섹션이 갈라진다는 주석을 양쪽에 달았다. 시드 8건에도 category를 부여했다
- [x] **어드민 드롭다운**: `FormControls.Select`(네이티브 select를 대체하는 공용 커스텀 드롭다운) 사용. "기타"를 고를 때만 직접 입력 칸이 나타나고, 다른 종류로 바꾸면 `toPayload`가 `category_etc`를 null로 비운다
- [x] **공개 페이지 섹션**: `groupByCategory`로 포스터 → 장서표 → 기타 고정 순서. 각 섹션은 종류명 h2 + 카드 그리드이고, 서버가 학기 내림차순으로 내려주므로 섹션 안 순서가 곧 최신순이다. category가 빈 레거시 행은 기타로 모아 화면에서 사라지지 않게 했다. 항목 0건인 섹션은 렌더하지 않는다
- [x] **학기 라벨 산출**: `utils/format.js`에 `semesterOf(date)`(1~6월 → `{연도}-1`, 7~12월 → `{연도}-2`)와 `semesterLabelOf(item)`(저장된 라벨 우선, 없으면 개최일에서 산출) 추가. 목록 카드와 상세 페이지가 같은 함수를 쓴다
- [x] **카드 구조 통일**: 모든 카드가 [학기 라벨(제목 위 eyebrow) → 제목 → 기간] 구조다. 캐릭터 공모전은 `semester_label`이 없어 학기가 비고 날짜만 아래에 따로 떨어져 있었는데, `semesterLabelOf`가 `created_at`에서 2026-1을 산출해 다른 카드와 같은 자리에 붙는다
- [x] **검증(실측)**: 공개 API 응답에 실제 렌더 로직을 재현해 대조 — **섹션 3개**(포스터 4 / 장서표 4 / 기타 1), 각 섹션 안 2026-1 → 2024-2 최신순, **학기 라벨 없는 카드 0건**. `semesterOf` 8케이스 자체 점검 통과(1월·6월 경계, 7월·12월 경계, ISO 문자열, null·빈값·비정상). category 저장 왕복 — 변경·원복 모두 정확히 반영. 서버 스모크 12/12, 클라 빌드·린트 통과
- [!] **`category_etc` 정리는 클라이언트 책임**: 종류를 기타에서 다른 값으로 바꿀 때 `category_etc`를 비우는 로직은 `toPayload`에 있다. 서버는 받은 값을 그대로 저장하므로 API를 직접 호출하면 기타가 아닌데도 값이 남을 수 있다. 화면 표시에는 쓰지 않아 영향은 없다
...
## 53_EXHIBITION_COPY — 전시회 접수 안내 문구 DB 분리
- [x] **저장 위치**: `site_settings.exhibitionCopy`(키-값 jsonb 단일 문서). site_settings는 key/value 구조라 ALTER가 필요 없다. `{ ko: {...}, en: {...} }` 형태이며 국문·영문을 각각 담는다
- [x] **편집 가능한 항목**: 온보딩(라벨·안내 본문·절차 패널 제목·절차 목록·유의사항 패널 제목·유의사항 목록·시작 버튼 라벨) + 접수 폼(작품명 안내 문구·작품 설명 placeholder)
- [x] **기본값 = 분리 이전 코드 원문**: `data/exhibitionCopy.js`에 `EXHIBITION_COPY_DEFAULT`로 옮겼다. 문장은 한 글자도 바꾸지 않았다. DB 값이 비면 여기로 떨어진다
- [x] **폴백 병합**: `exhibitionCopy(saved, lang)`이 저장값과 기본값을 항목 단위로 병합한다. 빈 문자열·공백만·빈 배열은 값이 없는 것으로 보고 기본값을 쓴다. 영문은 값이 없으면 국문으로 폴백(사이트 i18n 규칙과 동일)
- [x] **어드민 편집 UI**: `ExhibitionAdmin`에 "접수 안내 문구" 패널 신설. 절차는 `제목 | 설명` 한 줄에 한 단계, 유의사항은 한 줄에 한 항목으로 입력하고, 절차 번호는 순서에서 파생해 담당자가 단계를 늘려도 어긋나지 않는다
- [x] **권한**: `MANAGER_SETTING_KEYS`에 `exhibitionCopy` 추가 — 전시회 담당인 manager가 문구를 고칠 수 있다. 사이트 전역 설정은 여전히 admin+ 전용이다
- [x] **공개 반영**: `ExhibitSubmit`의 `STEPS`·`NOTES` 상수와 하드코딩 문구를 제거하고 DB 값을 읽는다. `ExhibitEdit`의 작품명 안내도 같은 값을 쓴다. 우리 변경이 만든 고아 import(`WORK_TITLE_HINT`)는 두 파일에서 제거했다(원본 상수는 `exhibitFormShared`에 그대로 남겨 둠)
- [x] **특수 기능 무변경**: 과목 선택(SubjectField)·회차 지정·기간 검증·학기 연동은 손대지 않았다. 텍스트만 분리했다
- [x] **검증(실측)**: manager 계정으로 `PUT /admin/settings`에 문구를 저장하니 `GET /settings/public`에 즉시 반영됨을 확인하고 원복했다. 폴백 병합 5케이스 자체 점검 통과(미설정·설정·부분 설정·공백만·빈 배열). 클라 빌드·린트 통과
- [!] **미검증(브라우저 도구 부재)**: 어드민에서 문구를 고친 뒤 공개 접수 페이지가 실제로 바뀌는 화면은 육안 확인하지 못했다. API 왕복과 병합 로직까지만 확인했다
- [!] **영문 입력 UI는 아직 없다**: 저장 구조는 `{ko, en}`를 모두 담지만 어드민 패널은 국문만 편집한다. 접수 플로우가 현재 국문 전용이라 영문 칸을 먼저 만들지 않았다. 필요해지면 같은 패널에 en 탭을 붙이면 된다

## 54_UI_FIX_6 — 상세 페이지 정비 + About 레이아웃 (UI 6건)
- [x] **1 전시 기간 박스 축소**: 피처드 전시의 "전시 기간" 패널이 세로 flex 컬럼의 자식이라 기본 stretch로 정보 영역 전체 폭을 먹고 있었다. `self-start` 한 클래스로 교차축을 콘텐츠 폭으로 줄였다(실측 약 635px → 약 350px). 상세 페이지의 학기·기간 표(`dl`)도 같은 이유로 `w-max max-w-full`을 줘 헤어라인이 텍스트 끝에서 멈춘다 — 전시회·공모전 상세 양쪽 동일
- [x] **2 전시 사이트 버튼 라벨·화살표**: 라벨을 `exhibitionSiteLabel(semester_label)`로 파생한다(`2026-1` → `26-1 DAH EXHIBITION`, 전시 사이트 도메인 표기와 같은 형식). 라벨이 안 나오는 전시(학기 라벨 없음)는 기존 `actions.exhibitionSite`로 폴백. 국·영문 공통 표기라 i18n 대역 없음. 화살표는 `Button`에 `arrow` prop(기본 true)을 달아 이 버튼만 끈다 — P5(외부 링크 ArrowUpRight)를 전역에서 깨지 않기 위해서다
- [x] **3 버튼 위치 고정**: 전시회·공모전 상세 모두 액션 버튼을 좌측 포스터 컬럼 아래로 옮겼다. 우측 정보 영역에 있으면 본문·메타 길이에 따라 버튼 높이가 글마다 달라진다. 포스터의 폭 상한(`max-w-[240px]`)은 figure에만 남겨 모바일에서 버튼이 눌리지 않게 했다
- [x] **4 동아리 상세**: 분야 "콘텐츠" Tag 알약을 제목 위 eyebrow(mono label, text-meta)로 바꿨다(P1 3단 위계). 사이트 버튼은 로고 아래에 이미 있었고 어드민 "사이트 URL" 입력 칸도 이미 있었다 — **버튼이 안 보인 원인은 코드가 아니라 데이터로, 동아리 4건 전부 `site_url`이 NULL이다**. 라벨을 "사이트 바로가기"에서 i18n `actions.clubSite`(웹사이트 / Website)로 바꾸고, Button primary 스타일을 그대로 베껴 쓰던 손수 만든 `<a>`를 공용 `Button`으로 교체했다
- [x] **5 미션 강제 줄바꿈**: `<br>`은 원래 없었다. 실제 원인은 h2의 `max-w-4xl`(896px)이라 "better" 뒤가 끊겼다. 폭 상한을 풀어 데스크탑 한 줄, 좁은 화면에서만 자연 줄바꿈
- [x] **6 About 레이아웃**: 01 OVERVIEW를 "What is DAH" / "Why DAH" 두 블록의 좌우 지그재그로 재배치했다(md 이상 2열, Why는 `md:order-2`로 헤딩이 오른쪽·모바일에서는 헤딩이 먼저). 헤딩은 h1 스케일 + Primary Purple 2px 밑줄 한 줄뿐이고, 배경·본문색은 기존 토큰 그대로다(레퍼런스의 흰 배경·검정 텍스트 미채택). 본문 키워드는 `Highlight`가 지정 구절만 `<strong>`으로 감싼다 — 문자열을 자르지 않고 split 캡처 그룹으로 나누므로 원문이 문자 단위로 보존된다. 원문 문구는 추가·삭제·수정 없음
- [x] **검증(헤드리스 크롬 실측)**: 로컬 API + preview 빌드를 Chrome headless로 띄워 6건 전부 화면으로 확인했다. 기간 박스 축소·버튼 라벨 `26-1 DAH EXHIBITION`·화살표 제거·포스터 하단 버튼(전시회/공모전)·About 지그재그·미션 한 줄. 동아리 웹사이트 버튼은 id 27에 `site_url`을 임시로 넣어 렌더를 확인한 뒤 NULL로 원복했다(4건 전부 NULL 재확인). `Highlight` 키워드 대조 자체 점검 — 국·영문 4개 문단 전부 원문 복원 일치, 키워드 중복·누락 0. `exhibitionSiteLabel` 7케이스 통과. 320px 실측은 iframe으로 했다(맥OS 창 최소 폭 때문에 `--window-size=320`이 실제로는 더 넓게 렌더된다) — 가로 넘침 0, 버튼 2줄 랩
- [!] **`md:mt-56` 함정**: 처음에 쓴 `md:mt-56`이 `spacing.scale`에 없는 값이라 Tailwind 기본 스케일의 14rem(224px)으로 잡혀 OVERVIEW 아래에 233px 공백이 생겼다. 실측으로 잡아 `md:mt-48`로 고쳤다. 커스텀 spacing은 기본 스케일과 **병합**되므로 스케일 밖 숫자도 조용히 통과한다 — 추가한 spacing 클래스를 전수 검사해 위반 0건 확인
- [!] **모바일 순서**: 상세 페이지가 1열이 되면 포스터 → 액션 버튼 → 제목 순이 된다. "버튼은 항상 포스터 바로 아래"라는 지시를 그대로 따른 결과다. 제목을 버튼 위로 올리려면 별도 지시가 필요하다

## 55_ABOUT_ZIGZAG_TIGHTEN — About 지그재그 좌우 간격·베이스라인 정렬
- [x] **원인**: 54에서 `md:grid-cols-2`로 만들어 제목 컬럼이 절반(576px)을 차지했다. "What is DAH"의 실제 잉크 폭은 약 207px이라 컬럼 안에서 369px이 죽고, 여기에 `md:gap-64`가 더해져 가운데가 433px 벌어졌다
- [x] **12컬럼 재배치**: `md:grid-cols-12` + 제목 `md:col-span-5` / 본문 `md:col-span-7`, 컬럼 갭은 `md:gap-32`. 가운데 공백 **433px → 314px**(1440 실측). 본문을 6칸이 아니라 7칸으로 둔 이유는 6칸이면 12번째 칸이 남아 본문 오른쪽 끝이 컨테이너보다 100px 짧아지기 때문이다 — 가운데 간격은 6칸일 때와 동일하고 오른쪽 여백만 없앤다
- [x] **베이스라인 정렬**: `md:items-baseline`. top 정렬은 제목(36px)과 본문(17px)의 글자 크기 차이만큼 본문 첫 줄이 위로 뜨고, 제목의 밑줄·`pb-12`가 더해져 어긋나 보였다. baseline 정렬은 밑줄을 계산에서 제외하므로 제목 첫 줄과 본문 첫 줄이 같은 선에 앉는다 — 실측 **베이스라인 차 1px**(제목 잉크 485~512 / 본문 첫 줄 498~513)
- [x] **섹션 간 수직 간격**: What↔Why 사이를 `mt-96 + pt-64`(160px) → `mt-64 + pt-48`(112px)로 줄였다. 구분 헤어라인은 유지해 두 섹션이 붙어 보이지 않는다
- [x] **유지**: 지그재그(What 제목 좌·본문 우 / Why 본문 좌·제목 우, `md:order-1/2`), Primary Purple 2px 밑줄, 키워드 볼드, 원문 문구 무변경. 그리드 자식에 `min-w-0`을 붙여 가로 넘침 규칙(11_DESIGN_V2 8절)도 맞췄다
- [x] **검증(320~3840 실측)**: 가운데 공백 — 320·390 1열(갭 없음, 제목 → 본문 순 확인), 768 **144px**, 1024 **230px**, 1440·1920·3840 **314px**(컨테이너가 1280에서 상한이라 그 이상은 동일). 전 구간 가로 넘침 0. 추가한 spacing 클래스는 `spacing.scale` 대조로 위반 0건(54에서 `md:mt-56`이 기본 스케일 14rem으로 잡혔던 함정 재발 방지). 클라 빌드·린트 통과
- [!] **Why 쪽은 반대로 가깝다**: 지그재그라 Why는 본문이 왼쪽·제목이 오른쪽인데, 본문 마지막 줄 끝(약 790px)과 "Why DAH" 시작(약 840px) 사이가 **약 50px**이다. 컬럼 갭 32px에 본문 줄 끝 여유가 더해진 값이고, 제목이 36px 볼드라 위계는 분명하다. 더 띄우려면 `md:gap-32`를 올려야 하는데 그러면 What 쪽 314px도 같이 커진다

## 56_ABOUT_ZIGZAG_EDGE — About 지그재그 제목 양끝 정렬 + 본문 가독 폭
- [x] **Why 제목 우측 끝 정렬**: 55에서 12컬럼 `col-span-5`로 잡았더니 제목이 컬럼 왼쪽에 붙어 오른쪽 경계에서 약 340px 떠 있었다. 제목 래퍼에 `md:text-right`를 줘 inline-block인 제목을 컬럼 우측 끝으로 민다. `SectionHeading`의 밑줄은 inline-block 자신의 border라 제목 폭 그대로 따라간다 — 별도 처리 불필요. What 제목은 좌측 정렬 유지
- [x] **본문 가독 폭**: 12컬럼(제목 5 / 본문 7 = 696px)에서 2등분 + `md:gap-32`로 되돌렸다. 컨테이너 콘텐츠 1216px 기준 컬럼이 **592px**로, 지시한 가독 폭 520~600 구간에 든다. 별도 `max-w`를 덧붙이지 않고 컬럼 폭이 곧 문단 폭이 되게 했다(중복 상한 방지). 국문·영문 동일
- [x] **양끝 정렬 원칙**: What = 제목 좌측 경계 + 본문 우측 경계, Why = 본문 좌측 경계 + 제목 우측 경계. 제목의 바깥쪽 끝이 콘텐츠 경계에 딱 맞고 가운데는 의도적으로 비운다(1440에서 What 418px, Why 517px)
- [x] **첫 줄 상단 정렬**: `md:items-baseline` → `md:items-start`. baseline 정렬은 제목(36px)과 본문(17px)의 half-leading 차이만큼 본문 첫 줄이 아래로 내려가 상단이 약 10px 어긋나 보였다. 상단 정렬로 **What 3px / Why 2px**까지 좁혔다(글꼴 어센트 차이라 0은 불가능 — 매직 넘버 오프셋은 유동 타이포에서 폭마다 틀어지므로 쓰지 않았다)
- [x] **검증(보라 밑줄 픽셀 실측, 320~3840)**: 밑줄 x범위로 양끝 정렬을 직접 쟀다 — 320 `16~165`/`16~130`(1열, 둘 다 좌측), 390 동일, 768 `24~194`/`614~743`, 1024 `32~216`/`851~991`, 1440 `112~319`/`1169~1327`, 1920 `352~559`/`1409~1567`, 3840 `1312~1519`/`2369~2527`. **전 구간에서 What 밑줄 왼쪽 끝 = 콘텐츠 좌측 경계, Why 밑줄 오른쪽 끝 = 콘텐츠 우측 경계**. 본문 폭 696 → **592px**. 첫 줄 상단차 3px/2px. 영문 페이지(`/en/about`)도 동일 확인. 클라 빌드·린트 통과
- [!] **1321 vs 1328**: 지시문의 우측 경계 1321은 국문 본문 글자의 **잉크** 오른쪽 끝이고, 실제 콘텐츠 박스 경계는 1328이다(한글 글리프 사이드 베어링 차이). Why 제목 밑줄은 박스 경계인 1327~1328에 맞췄다 — 타이포그래피상 이쪽이 정확한 flush다
